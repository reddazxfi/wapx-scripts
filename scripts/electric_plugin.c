...
void CMine::TazeMine()
{
    ...
    tazed_m = true;                
    linkedEffect1 = new CEffectManager(this, PosX, PosY, 99, 15, 15, 4.0, 130, 180, 255);  
    linkedEffect1->glow = false;        
    linkedEffect1->SetTrail( 12, 2, 0);
    
    linkedEffect2 = new CEffectManager(this, PosX, PosY, 99, 15, 15, 4.0, 130, 180, 255);         
...
override void CMine::Message(CObject* sender,EMType Type,int MSize,CMessageData* MData)
{
    ...
    if (!tazed_m && ClType != OC_Mine) return;
                              
    if (Type == M_DRAWQUEUE && tazed_m)
    {
    ...
    {
        if (linkedEffect1 == NullObj) { return;}  //should never happen
        ...
        if (linkedEffect2 == NullObj){ return;  }
